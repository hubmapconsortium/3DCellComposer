...
def write_to_obj(verts, faces, groups, colors, filename):
    with open(filename, "w") as f:
        f.write(f"mtllib ./data/cell_mesh.mtl\n")
...
def meshing_3D(mask, mask_colored, num_of_col, output_path):
    ...
    all_colors = []
    offset = (
        0  # To keep track of the index offset for faces when combining multiple cells
    )
    ...
    for cell_index in cell_coords.index:
        ...
        if len(np.unique(current_coords[0])) >= 2:
            ...
            if np.sum(current_mask[z_start] != 0) > np.sum(
                current_mask[z_start + 1] != 0
            ):
                start_slice_mesh = get_2D_mesh(current_mask[z_start])
                start_2D_contours = [
                    convert_2D_contour_to_3D(contour, z_start)
                    for contour in start_slice_mesh
                ]
                start_triangles = [
                    triangulate_2D_contour(contour) for contour in start_2D_contours
                ]
            ...
            if np.sum(current_mask[z_end] != 0) > np.sum(current_mask[z_end - 1] != 0):
                ...
                end_2D_contours = [
                    convert_2D_contour_to_3D(contour, z_end)
                    for contour in end_slice_mesh
                ]
                end_triangles = [
                    triangulate_2D_contour(contour) for contour in end_2D_contours
                ]
    ...
    write_to_obj(
        all_verts, all_faces, all_groups, all_colors, f"{output_path}/cell_mesh.obj"
    )
