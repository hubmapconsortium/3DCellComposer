...
def process_segmentation_masks(
    ...
    for JI in JI_range:
        ...
        matched_3D_cell_mask = matching_cells_3D(
            matched_2D_stack_XY, matched_2D_stack_XZ, matched_2D_stack_YZ
        )
...
def ThreeDCellComposer(
    ...
    if segmentation_method in ["deepcell", "custom"]:
        ...
        if segmentation_method == "deepcell":
            ...
            nuclear_mask_all_axes = {}
            for axis in ["XY", "XZ", "YZ"]:
                cell_mask_axis, nuclear_mask_axis = deepcell_segmentation_2D(
                    nucleus_channel, membrane_channel, axis, voxel_size
                )
    ...
    image_labels = [
        f"{i}" for i in range(combined_mask.shape[0] * combined_mask.shape[1])
    ]
    tifffile.imwrite(
        f"{results_path}/3D_cell_nuclear_mask.tif",
        swapped,
        imagej=True,
        resolution=(1.0 / float(voxel_size[0]), 1.0 / float(voxel_size[1])),
        metadata={
