...
from segmentation_2D.installation.install_all_methods import (
...
def process_segmentation_masks(
    ...
    if os.path.exists(m3dfilename):
        ...
        for JI in JI_range:
            ...
            matched_3D_cell_mask = matching_cells_3D(
                matched_2D_stack_XY, matched_2D_stack_XZ, matched_2D_stack_YZ, minslices
            )
    ...
    if os.path.exists(m3dbfilename):
        with open(m3dbfilename, "rb") as ss_file:
            final_matched_3D_cell_mask_JI, final_matched_3D_nuclear_mask_JI = (
                pickle.load(ss_file)
            )
    else:
        ...
        for JI in JI_range:
            ...
            final_matched_3D_cell_mask, final_matched_3D_nuclear_mask = (
                matching_nuclei_3D(matched_3D_cell_mask, nuclear_mask_all_axes["XY"])
            )
            ...
            # print(final_matched_3D_cell_mask.shape,final_matched_3D_nuclear_mask.shape)
            final_matched_3D_cell_mask_JI[JI] = upsamplemask(
                final_matched_3D_cell_mask, downsample_vector, image.shape
            )
            final_matched_3D_nuclear_mask_JI[JI] = upsamplemask(
                final_matched_3D_nuclear_mask, downsample_vector, image.shape
            )
    ...
    if skip_eval:
        ...
        try:
            for JI in JI_range:
                ...
                writeresults(
                    savepath,
                    final_matched_3D_cell_mask_JI[JI],
                    final_matched_3D_nuclear_mask_JI[JI],
                    best_metrics,
                    best_quality_score,
                )
                JIcount += 1
...
def main():
    ...
    parser.add_argument(
        "--segmentation_method",
        type=str,
        choices=["deepcell", "cellpose", "custom"],
        default="deepcell",
    ...
    parser.add_argument(
        "--chunk_size", type=int, default=100, help="Chunk size for segmentation"
    )
    parser.add_argument(
        "--sampling_interval",
        type=parse_marker_list,
    ...
    parser.add_argument(
        "--skip_eval", type=bool, default=False, help="Skip CellSegmentationEvaluator"
    )
    parser.add_argument(
        "--skip_blender", type=bool, default=False, help="Skip generating blender files"
    )
    parser.add_argument(
        "--downsample_vector",
        type=parse_marker_list,
    ...
    parser.add_argument("--skipYZ", type=bool, default=False, help="skip YZ slicing")
    parser.add_argument(
        "--clear_cache",
        type=bool,
        default=False,
    ...
    nucleus_channel, cytoplasm_channel, membrane_channel, image = (
        write_IMC_input_channels(
            image_path,
            args.results_path,
    ...
    print(
        f"Marker channels shape: {nucleus_channel.shape}, All channels shape: {image.shape}"
    )
    ...
    if any(x != 1 for x in downsample_vector):
        voxel_down = (
            vsi[0] * downsample_vector[0],
            vsi[1] * downsample_vector[1],
            vsi[2] * downsample_vector[2],
        )
        print(f"After downsample voxel size Z,Y,X: {voxel_down}")

        nucleus_down = block_reduce(
            nucleus_channel, block_size=downsample_vector, func=np.max
        )
        cytoplasm_down = block_reduce(
            cytoplasm_channel, block_size=downsample_vector, func=np.max
        )
        membrane_down = block_reduce(
            membrane_channel, block_size=downsample_vector, func=np.max
        )
    ...
    if args.segmentation_method in ["deepcell", "cellpose", "custom"]:
        ...
        while max_tries > 0:
            ...
            nuclear_mask_all_axes = {}
            for axis in axestouse:
                cell_mask_axis, nuclear_mask_axis = deep_segmentation_2D(
                    args.segmentation_method,
                    nucleus_down,
                    membrane_down,
                    axis,
                    voxel_down,
                    sampling_interval[axis],
                    args.chunk_size,
                    args.results_path,
                    args.maxima_threshold,
                    args.interior_threshold,
                    args.compartment,
                    args.min_slice_padding,
                )
            ...
            if not args.skip_eval:
                print(
                    f"Quality Score of this 3D Cell Segmentation = {best_quality_score}"
                )
                if max(sampling_interval.values()) > 1:
                    trialpath = args.results_path / (
                        "trial" + str(args.max_tries - max_tries)
                    )
                    writeresults(
                        trialpath,
                        best_cell_mask_final,
            ...
            if (
                best_quality_score > args.quality_threshold
                or max(sampling_interval.values()) == 1
                or max_tries == 1
            ):
                break
            else:
                ...
                for axis in ["XY", "XZ", "YZ"]:
                    sampling_interval[axis] = int(
                        max(sampling_interval[axis] / sampling_reduce, 1)
                    )
                print(
                    f"Quality score is too low, Sampling interval is reduced to {sampling_interval}"
                )
        ...
        for method in all_methods:
            cell_mask_all_axes, nuclear_mask_all_axes = segmentation_single_method(
                method, image_path.parent, voxel_down
            )
            (
                method_quality_score,
                method_metrics,
                method_cell_mask_final,
                method_nuclear_mask_final,
            ) = process_segmentation_masks(
                cell_mask_all_axes,
                nuclear_mask_all_axes,
                nucleus_channel,
    ...
    if not args.skip_blender:
        ...
        best_cell_mask_final_colored, number_of_colors = coloring_3D(
            best_cell_mask_final
        )
        meshing_3D(
            best_cell_mask_final,
            best_cell_mask_final_colored,
            number_of_colors,
            args.results_path,
        )
