...
def deep_segmentation_2D(
    ...
    print(
        f"Segmenting in {axis} direction with sampling interval {sampling_interval}..."
    )
    ...
    for i in desired_slices:
        if not isinstance(saved_segmentations[i], np.ndarray):
            ...
            if not i % chunk_size:
                print(
                    f"Segmented slice {i} of {len(im)}: #cells={len(np.unique(pred))}"
                )
            if i == desired_slices[0]:
                tend = time.time()
                etime = tend - tstart
                esttime = etime * (len(desired_slices) - 1) / 60.0
                print(
                    f"Elapsed time: {round(etime/60.,2)} min; estimated remaining time for this axis: {round(esttime,2)} min"
                )
...
def pred_deepcell(
    app, slice, pixel_size, compartment, interior_threshold, maxima_threshold
):
