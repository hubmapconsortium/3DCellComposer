...
def combine_slices(method, img_dir):
    ...
    for axis in axes:
        ...
        slice_dir_list = sorted(
            glob.glob(f"{img_dir}/slices/slice_{axis}*", recursive=True),
            key=extract_number,
        )
        for slice_dir in slice_dir_list:
            try:
                cell_mask_slice = pickle.load(
                    bz2.BZ2File(f"{slice_dir}/cell_mask_{method}.pkl", "r")
                )
            except:
                cell_mask_slice = np.zeros(slice_shape)
            try:
                nuclear_mask_slice = pickle.load(
                    bz2.BZ2File(f"{slice_dir}/nuclear_mask_{method}.pkl", "r")
                )
            except:
    ...
    for img_name in img_names:
        ...
        for axis in axes:
            ...
            for slice_index in range(img_rotated.shape[0]):
                ...
                slice_dir = join(
                    img_dir, "slices", "slice_" + str(axis) + "_" + str(slice_index)
                )
...
def segmentation_single_method(method, img_path, voxel_size):
    ...
    if method in [
        ...
        subprocess.run(
            [f"{current_dir}/segmentation_2D/{run_file}", img_path, voxel_size]
        )
    elif method in ["CellProfiler", "ACSS_classic", "CellX", "CellSegm"]:
        axes = ["XY", "XZ", "YZ"]
        for axis in axes:
            slice_paths = sorted(
                glob.glob(f"{img_path}/slices/slice_{axis}*", recursive=True)
            )
            if axis == "XY":
            ...
            for slice_path in slice_paths[:1]:
                ...
                subprocess.run(
                    [
                        f"{current_dir}/segmentation_2D/{run_file}",
                        slice_path,
