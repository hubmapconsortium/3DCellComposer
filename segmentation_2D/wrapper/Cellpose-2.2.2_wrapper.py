...
for z in range(im.shape[1]):
    masks, flows, styles = model.eval(
        im[:, z, :, :], diameter=10 / pixel_size_ratio, channels=channels
    )
    masks_3D_pieces.append(masks)
masks_3D = np.stack(masks_3D_pieces, axis=0)
pickle.dump(
    masks_3D, bz2.BZ2File(f"{file_dir}/cell_mask_Cellpose-2.2.2_{axis}.pkl", "w")
)
...
for z in range(im.shape[1]):
    masks, flows, styles = model.eval(
        im2[:, z, :, :], diameter=8 / pixel_size_ratio, channels=channels
    )
...
pickle.dump(
    nuclear_masks_3D,
    bz2.BZ2File(f"{file_dir}/nuclear_mask_Cellpose-2.2.2_{axis}.pkl", "w"),
)
