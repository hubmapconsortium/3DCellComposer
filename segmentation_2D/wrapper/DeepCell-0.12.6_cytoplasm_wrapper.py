...
if axis == "XY":
    ...
    im_zeros = np.zeros(
        (im.shape[0], im.shape[1], im.shape[0] - im.shape[2] - 300, im.shape[3])
    )
    im = np.dstack((im, im_zeros))
elif axis == "YZ":
    im_zeros = np.zeros(
        (im.shape[0], im.shape[2] - im.shape[1] - 300, im.shape[2], im.shape[3])
    )
...
for i in range(len(im)):
    if i == 0:
        labeled_image = app.predict(
            np.expand_dims(im[i], 0), image_mpp=pixel_size, compartment="both"
        )
    else:
...
mask_dir = bz2.BZ2File(
    join(file_dir, "cell_mask_DeepCell-0.12.6_cytoplasm_" + axis + ".pkl"), "wb"
)
pickle.dump(cell_mask, mask_dir)

nuc_mask_dir = bz2.BZ2File(
    join(file_dir, "nuclear_mask_DeepCell-0.12.6_cytoplasm_" + axis + ".pkl"), "wb"
)
