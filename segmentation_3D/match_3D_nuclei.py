    ...
    for i in cell_coords.index:
        if len(cell_coords[i]) != 0 and np.unique(nuclear_slice)[0] != 1:
            ...
            for j in nuclear_search_num:
                if j != 0:
                    if (j not in nucleus_matched_index_list) and (
...
def trim_nuclei_z_slice(cell_mask, nuclear_mask):
    ...
    for cell_idx in cell_coords.index:
        ...
        if not nuclear_all_zeros:
            ...
            nuclear_start_slice_array_idx = np.where(
                current_nucleus_coords[:, 0] == cell_start_slice_idx
            )
            current_nucleus_coords = np.delete(
                current_nucleus_coords, nuclear_start_slice_array_idx, axis=0
            )
            nuclear_end_slice_array_idx = np.where(
                current_nucleus_coords[:, 0] == cell_end_slice_idx
            )
            current_nucleus_coords = np.delete(
                current_nucleus_coords, nuclear_end_slice_array_idx, axis=0
            )
...
def matching_nuclei_3D(cell_mask_3D, nuclear_slice):
    ...
    for slice_idx in range(nuclear_slice.shape[0]):
        # print(slice_idx)
        repaired_nuclear_slice = match_repair_cell_nucleus(
    ...
    matched_cell_3D, matched_nuclear_3D = trim_nuclei_z_slice(
        cell_mask_3D, matched_nuclear_3D
    )
