...
def matching_cells_2D(img, JI_thre):
    ...
    for slice_num in range(1, img.shape[0]):
        ...
        current_slice_cell_coords = list(
            map(lambda x: np.array(x).T, current_slice_cell_coords)
        )
        new_slice_cell_coords = list(
            map(lambda x: np.array(x).T, new_slice_cell_coords)
        )
        ...
        for i in range(len(current_slice_cell_coords)):
            if len(current_slice_cell_coords[i]) != 0:
                new_slice_search_num = np.unique(
                    list(
                        map(
                            lambda x: img_new_slice[tuple(x)],
                            current_slice_cell_coords[i],
                        )
                    )
                )
                best_JI = 0
        ...
        new_slice_cell_unmatched_list, new_slice_cell_unmatched_index_list = (
            get_unmatched_list(new_slice_cell_matched_index_list, new_slice_cell_coords)
        )
