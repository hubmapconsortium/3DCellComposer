...
def matching_cells_3D(mask_XY, mask_XZ, mask_YZ, minslices):
    ...
    sorted_cells = sorted(
        cell_volumes.keys(), key=lambda x: cell_volumes[x], reverse=True
    )
    ...
    for cell_index in sorted_cells:
        ...
        if (
            not np.any(segmentation_XY_repaired_binary[current_coords])
            and len(np.unique(current_coords[0])) >= minslices
        ):
    ...
    segmentation_XY_repaired_output = np.zeros(
        segmentation_XY_repaired.shape, dtype=int
    )
    ...
    for coord_idx in segmentation_XY_repaired_coords.index:
        segmentation_XY_repaired_output[segmentation_XY_repaired_coords[coord_idx]] = (
            cell_idx
        )
        cell_idx += 1
